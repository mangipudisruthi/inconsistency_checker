<?php if(!defined("BASEPATH")){ exit("No direct script access allowed"); }
	...
	**/
	class MY_Controller extends CI_Controller {
	    	...
	    	foreach($auths as $i => $auth)
	    	{
	    		...
	    		foreach($modules as $row)
	    		{
	    			...
	    			if(file_exists(APPPATH."models/$module/".$module."_menu_model.php"))
	    			{
	    				...
	    				if($module == 'information' && $pos!= false)
	    				{
	    					$model_menu = $this->$module->getMenu($auth);
	    					 	if(isset($model_menu['da1']) && is_array($model_menu['da1']))
	    					 	{
	    					 		$menu[$auth] = array_merge($menu[$auth], $model_menu['da1']);
		...
		// The following header is used to disable menu from LHS of Draw header as mentioned above. 
		// This is used at present for security question settings
		function sec_Header($title = "Management Information System", $subtitle = "") {
			$ss=array();
			$data = array("menu" => $ss,
						  "title" => $title,
						  "subtitle" => $subtitle,
